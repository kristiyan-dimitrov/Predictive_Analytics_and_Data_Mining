...
Downloaded data from: https://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/
...
for (i in colnames(redwine)){
  ...
  if (round(Kurt(redwine[,i], na.rm = TRUE),4) < 2.5){
    print(paste(i, "is platykurtic"))
  } 
  
  if(round(Kurt(redwine[,i], na.rm = TRUE),4) > 3.5){
    print(paste(i, "is leptokurtic"))
  }
  
   if(round(Kurt(redwine[,i], na.rm = TRUE),4) < 3.5 && round(Kurt(redwine[,i], na.rm = TRUE),4) > 2.5){
...
- Looking at the Q-Q plots for fixed.acidity and volatile acidity, we see that 1) both tails deviate from the normal line, which designates skinnier tails than the normal distribution and therefore Kurtosis < 3 i.e. platykurtic and 2) the distributions appear to deviate from the normal distribution "by the same amount" both in the top and lower range, which is precisely designated by a low skewness (compared to all the other vairables)
...
for (j in seq(1,5)){ # Take each fold of data one at a time
  ...
  for(i in seq(1,ncol(testFold))){ # Take one column at a time from the testFold
    ...
    testFold[,i][missing] <- rep(avg, n.missing) # replacing the missing values with the mean
...
for (j in seq(1,5)){ # Take each fold of data one at a time
  ...
  testFold[missingQA,]$QA = testFold.mice$imp$QA[,5]
...
- Of course, it probably just matters on your specific dataset.
